...
class NamedVector:
    ...
    def get_named_value(self, key):
        return self._vector[self._get_index(key)]

    def set_named_value(self, key, value):
        self._vector[self._get_index(key)] = value
...
class NamedMatrix:
    ...
    def __repr__(self):
        return "_row_indices: %s; _col_indices: %s; _matrix: %s" % (str(self._row_indices), str(self._col_indices), str(self._matrix))
