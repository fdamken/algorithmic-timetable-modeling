...
\begin{document}
	...
	\subsection{Das Optimierungsproblem}
		...
		\paragraph{Dualität zum Minimum-Cost-Flow Problem}
			...
			Es kann gezeigt werden, dass jeder Extrempunkt dieses Problems einer Spannbaumstruktur \( \mathcal{T} = \mathcal{T}^l + \mathcal{T}^u \) entspricht, wobei die Spannung von jeder Kante in \( \mathcal{T}^l \) auf die untere und in \( \mathcal{T}^u \) auf die obere Schranke festgesetzt wird. Diese Spannbaumstrukturen generieren dann ein Potential (einen Fahrplan) \(\vec{\pi}\).
		% end
	% end
\end{document}
