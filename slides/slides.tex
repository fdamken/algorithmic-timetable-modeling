...
% Other packages.
\usepackage[english, linesnumbered, ruled]{algorithm2e}
\usepackage{calc}
\usepackage{csquotes}
\usepackage{float}
	...
	\section{Zusammenfassung}
		\begin{frame}{Der Modulo-Netzwerk-Simplex-Algorithmus}
			\begin{algorithm}[H]
				...
				\textit{Knotenlokale Verbesserungen:} \While{Es existiert eine Knotenlokale Verbesserung \(\vec{\eta}\)}{
					Anwendung des Schnitts durch Transformation der Lösung: \( \vec{y} \gets \vec{y} + \vec{\eta} \).
					
					Fixieren der Modulo-Parameter \(\vec{z}\).
					
					Lösung des Minimum-Cost-Flow Problems.
					
					Transformtion der Lösung in eine Spannbaumstruktur.
					
					\textit{Modulo-Simplex-Pivoting:} \While{Es existiert ein Paar \( (i, j) \) mit \( \Delta\omega_{i,j} < 0 \)}{
