				...
				\begin{minipage}{0.5\textwidth}
					\begin{figure}
						...
						\begin{tikzpicture}[event/.style = { draw, circle }, anchor = center]
							...
							\draw [->] (B) to node[midway, sloped, above]{\scriptsize \( a_1, [9, 28] \)} (A);
							\draw [->] (B) to node[midway, sloped, above]{\small \( a_2, [7, 26] \)} (C);
							\draw [->] (D) to node[midway, sloped, above]{\small \( a_3, [2, 21] \)} (A);
							\draw [->] (B) to node[midway, sloped, above]{\small \( a_4, [1, 20] \)} (D);
							\draw [->] (D) to node[midway, sloped, above]{\small \( a_5, [5, 24] \)} (C);
							\draw [->] (A) to node[midway, sloped, above]{\small \( a_6, [3, 22] \)} (C);
						\end{tikzpicture}
			...
			\begin{frame}{Beispiel, Basis-Austausch}
				...
				\begin{minipage}{0.5\textwidth}
					\begin{figure}
						\centering
						\begin{tikzpicture}[event/.style = { draw, circle }, spanning/.style = { draw = tud9b }, co/.style = { draw = tud2b }, anchor = center]
							\node [event, label = below:{ \( \pi_A =  \) }] (A) at (-2.3, -2.5) {\(A\)};
							\node [event, label = left:{ \( \pi_B =  \) }] (B) at (0, 0) {\(B\)};
							\node [event, label = right:{ \( \pi_C =  \) }] (C) at (-2.3, 2.5) {\(C\)};
							\node [event, label = below:{ \( \pi_D =  \) }] (D) at (2.5, 0) {\(D\)};
