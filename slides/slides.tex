...
\subtitle{Basierend auf~\cite{nachtigallSolvingPeriodicTimetable2008}}
...
\newcommand{\exampleDirectedGraph}{
	\begin{tikzpicture}[->, node/.style = { draw, rectangle, rounded corners, minimum width = 1cm, minimum height = 0.6cm }]
		\node [node] (a) {\rom{1}};
		\node [node, right = of a] (b) {\rom{2}};
		\node [node, below = of a] (c) {\rom{3}};
		\node [node, below = of b] (d) {\rom{4}};
...
\begin{document}
	...
	\section{Einleitung und Motivation}
		\begin{frame}{Einleitung}
			\begin{itemize}
				...
				\item Ziel: Optimierung dieser Fahrpläne, um Pufferzeiten (Slack-Zeiten, "Sinnlose Wartezeiten") zu minimieren.
			\end{itemize}
		\end{frame}
	
		\begin{frame}{Idee}
			\begin{itemize}
				\item Fokus auf die Optimierung von gewichteten Slack-Zeiten \\\(\to\) Gemischt-Ganzzahliges Programm.
	...
	\section{Mathematische Modellierung}
			...
			\begin{frame}{Aussagekraft von Zeiteinschränkungen}
				\begin{itemize}
					...
					\begin{itemize}
						...
						\item<2-> Betriebssicherheitseinschränkungen (\zB Vorfahrt)
		...
		\subsection{Taktfahrpläne}
			...
			\begin{frame}{Modulo-Operator und Slack-Zeiten}
				...
				\vspace{1cm}
				Die untere und obere \emph{Slack-Zeit} misst, um wie viel Zeit die Spannung \( x_a = \pi_j - \pi_i \) verändert werden darf, ohne ungültig zu werden:
				\begin{align*}
			...
			\begin{frame}{Das Optimierungsproblem (Potential-Modell)}
				\begin{itemize}
					\item Durch Umkehrung einer Kante \(a\) können obere und untere Slack-Zeit ausgetauscht werden.
			...
			\begin{frame}{Zusammenfassung}
				\begin{itemize}
					\item Taktzeit                \tabto{4cm} \( T = \const \)
					\item Fahrplan                \tabto{4cm} \( \vec{\pi} = (\pi_i) \)
					\item Vorgang                 \tabto{4cm} \( a : i \to j \)
					\item Spannung                \tabto{4cm} \( x_a = \pi_j - \pi_i \)
					\item Zulässige Dauer         \tabto{4cm} \( \Delta_a = [l_a, u_a] \)
					\item Untere/Obere Slack-Zeit \tabto{4cm} \( y_a^\mathit{low} = [x_a - l_a]_T \),\, \( y_a^\mathit{upp} = [u_a - x_a]_T \)
	...
	\section{Alternative Modellierung}
		\subsection{Grundlagen von Flussnetzwerken}
			\begin{frame}{Notation und Laufendes Beispiel}
				\begin{figure}
					\centering
					\exampleDirectedGraph
					\caption{Gerichteter Graph mit \( n = 5 \) Kanten und \( m = 4 \) Knoten (laufendes Beispiel)}
					\label{fig:directed_graph}
				\end{figure}
			
				Notation: Für "Die Kante \(a\) geht von Knoten \rom{2} zu Knoten \rom{1}", schreibe
				\begin{equation*}
					a : \text{\rom{2}} \to \text{\rom{1}}
				\end{equation*}
			\end{frame}
			...
			\begin{frame}{Inzidenzmatrix}
				Die \emph{Inzidenzmatrix} eines Graphen/Netzwerks mit \(n\) Knoten und \(m\) Kanten ist eine \( n \times m \)-Matrix \( \mat{\Theta} = (\theta_{ia}) \):
				\begin{equation*}
					\theta_{ia} \coloneqq
						\begin{cases*}
							1  & falls \( a : j \to i \) \\
			...
			\begin{frame}{Potential und Spannung}
				...
				\onslide<2->{
					...
					\vspace{-0.5cm}
					\begin{align*}
						\vec{\pi} &= \begin{bmatrix} \pi_\text{\rom{1}} & \pi_\text{\rom{2}} & \pi_\text{\rom{3}} & \pi_\text{\rom{4}} \end{bmatrix}^T \in \R^4 \\
						\vec{x} &=
							\begin{bmatrix}
								x_a \\
								x_b \\
						...
						\coloneqq
							\begin{bmatrix}
								\pi_\text{\rom{1}} - \pi_\text{\rom{2}} \\
								\pi_\text{\rom{3}} - \pi_\text{\rom{1}} \\
								\pi_\text{\rom{1}} - \pi_\text{\rom{4}} \\
								\pi_\text{\rom{4}} - \pi_\text{\rom{2}} \\
								\pi_\text{\rom{4}} - \pi_\text{\rom{3}}
							\end{bmatrix}
						=
							\underbrace{
								\begin{bmatrix}
			...
			\begin{frame}{Schnitte}
				...
				\onslide<2->{
					...
					\begin{minipage}[l]{0.7\textwidth}
						\begin{align*}
							P &= \{\, \text{\rom{3}} \,\} \\
							\bar{P} &= \{\, \text{\rom{1}}, \text{\rom{2}}, \text{\rom{4}} \,\} \\
		...
		\subsection{Spannbaumstrukturen}
			...
			\begin{frame}{Zusammenfassung}
				\begin{itemize}
					\item Spannbaumstruktur \tabto{3cm} \( \mathcal{T} = \mathcal{T}^l + \mathcal{T}^u \)
					\item \( \vec{b} = [-\mat{\Gamma}\vec{l}]_T \)
					\item \( \vec{\delta} = \vec{u} - \vec{l} \)
					\item Zulässige Slack-Zeiten \tabto{3cm}
				\end{itemize}
				\begin{equation*}
					\mathcal{Y} = \,\big\{\, \vec{y} \in \Z^m \,\vert\, \mat{\Gamma} \vec{y} \equiv_T \vec{b}, \quad \vec{0} \leq \vec{y} \leq \vec{\delta} \,\big\}
				\end{equation*}
				\begin{itemize}
					\item Optimierungsproblem
				\end{itemize}
		...
		\subsection{Untersuchung des Slack-Modells}
 			\begin{frame}{Dualität zum Minimum-Cost Flow Problem}
				Werden die Modulo-Parameter \(\vec{z} = (z_a)\) fixiert, kann das Optimierungsproblem umformuliert werden:
				\begin{align*}
					&\, \min \Bigg\{\, \sum_{a}^{\mathcal{A}} \omega_a (x_a - l_a - z_a T) \,\bigg\vert\, \forall a \in \mathcal{A} : l_a \leq x_a - z_a T \leq u_a \,\Bigg\} \\
					=&\, \min \Bigg\{\, \sum_{a}^{\mathcal{A}} \omega_a (x_a - l_a') \,\bigg\vert\, \forall a \in \mathcal{A} : \underbrace{l_a + z_a T}_{l_a'} \leq x_a \leq \underbrace{u_a + z_a T}_{u_a'} \,\Bigg\} \\
		...
		\subsection{Simplex-Tableau}
			\begin{frame}{Idee}
				\begin{itemize}
					...
					\item Keine Verbesserung möglich \(\to\) lokales Minimum gefunden.
				\end{itemize}
			\end{frame}
       ...
       &                                       &        &                        &         &        &        &     &        &        &                                 &  \omega
			...
			\begin{frame}{Auswahl des Pivotelements}
				\begin{itemize}
					...
					\item Ist keine negative (verbessernde) Änderung möglich \(\to\) Abbruch.
				\end{itemize}
			\end{frame}
...
%					\centering
%					\begin{tikzpicture}[event/.style = { draw, circle }, spanning/.style = { draw = tud9b }, co/.style = { draw = tud2b }, anchor = center]
%						\node [event, label = below:{ \( \pi_A = 9 \) }] (A) at (-2.3, -2.5) {\rom{1}};
%						\node [event, label =  left:{ \( \pi_B = 0 \) }] (B) at (0, 0) {\rom{2}};
%						\node [event, label = right:{ \( \pi_C = 12 \) }] (C) at (-2.3, 2.5) {\rom{3}};
%						\node [event, label = below:{ \( \pi_D = 1 \) }] (D) at (2.5, 0) {\rom{4}};
%						
%						\draw [->, spanning] (B) to node[midway, sloped, above, yshift = -2pt]{\scriptsize \( a_1, [9, 28] \)} (A);
%						\draw [->, co]       (B) to node[midway, sloped, above, yshift = -2pt]{\scriptsize \( a_2, [7, 26] \)} (C);
...
%			\end{frame}
			...
			\begin{frame}{Lokalität des Verfahrens}
				\begin{itemize}
					\item Das Simplex-Verfahren findet meistens nur ein lokales Minimum.
					\item Es sind Modifikationen nötig, um ein globales Optimum zu finden.
				\end{itemize}
			\end{frame}
		% end
		
		\subsection{Verbesserungen der Lösung}
			\begin{frame}{Änderung der Lösung durch Spannungen}
				...
				\end{equation*}
%				\begin{equation*}
%					\mat{\Gamma} \vec{y}' \equiv_T \vec{b} \,\iff\, \mat{\Gamma} (\vec{y} + \vec{x}) \equiv_T \vec{b} \,\iff\, \mat{\Gamma}\vec{y} + \mat{\Gamma}\vec{x} \equiv_T \vec{b} \,\iff\, \mat{\Gamma}\vec{y} \equiv_T \vec{b}
%				\end{equation*}
			...
			\begin{frame}{Knotenlokale Verbesserungen}
				Ein Knoten \(i\) induziert einen Schnitt \( \vec{\eta}^{(i)} \) mit \( P = \{\, i \,\} \). Eine Änderung des Potentials \( \pi_i \)
				\begin{equation*}
					\pi_i' \coloneqq \pi_i + \delta
				\end{equation*}
				ist äquivalent zur Änderung der Slack-Zeiten
				\begin{equation*}
					\vec{y}' \coloneqq \big[ \vec{y} + \delta\vec{\eta}^{(i)} \big]_T
				\end{equation*}
	...
	\section{Zusammenfassung und Abschluss}
		\begin{frame}{Der Modulo-Netzwerk-Simplex-Algorithmus}
			\begin{algorithm}[H]
				\textit{Initialisierung:} Bestimmung einer initialen Spannbaumstruktur \( \mathcal{T} = \mathcal{T}^l + \mathcal{T}^u \) mit Lösung \( \vec{y} \).
				
				\BlankLine
				\While{Es existiert eine Knotenlokale Verbesserung \(\vec{\eta}\).}{
					Anwendung des Schnitts durch Transformation der Lösung: \( \vec{y} \gets [\vec{y} + \vec{\eta}]_T \).
					
					Fixieren der Modulo-Parameter \(\vec{z}\), Lösung des Minimum-Cost Flow Problems und Transformtion der Lösung in eine Spannbaumstruktur.
					
					\BlankLine
					\While{Es existiert ein Paar \( (i, j) \) mit \( \Delta\omega_{i,j} < 0 \).}{
						Tausche Ko-Baum-Kante \( a_i \) mit Baum-Kante \( a_j \).
					}
				}
			\end{algorithm}
		\end{frame}
	
		\begin{frame}{Globalität der Lösung}
			\begin{itemize}
				\item Nur lokales Optimum.
				\item Knotenlokale Verbesserungen möglich.
				\item Dies garantiert aber noch immer kein globales Optimum!
				\item Weitere Möglichkeiten (siehe~\cite{goerigkEngineeringModuloNetwork2011}):
					\begin{itemize}
						\item Waiting Edge Cuts
						\item Random Node Cuts
						\item Multi Node Cuts
					\end{itemize}
			\end{itemize}
		\end{frame}
	% end
