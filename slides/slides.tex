	...
	\section{Das Slack-Modell} % TODO: Title.
		...
		\subsection{Spannbaumstrukturen}
			...
			\begin{frame}{Das Optimierungsproblem (Slack-Modell)}
				...
				\end{align*}
				Damit ist \( \vec{x} \in \Z^m \) eine Spannung mit \( \vec{l} \leq \vec{x} \leq \vec{u} \) gdw. \( (\vec{x} - \vec{l}) \in \mathcal{Y} \) gilt. Also bestimmt jedes \( \vec{y} \in \mathcal{Y} \) eine Spannung \( \vec{x} \coloneqq \vec{y} + \vec{l} \) mit \( \vec{l} \leq \vec{x} \leq \vec{u} \).
...
%			}
	...
	\section{Modulo-Simplex-Berechnungen}
		...
		\subsection{Modulo-Simplex-Tableau}
			\begin{frame}{Idee}
				\begin{itemize}
			...
			\begin{frame}{Separierung der Netzwerkmatrix}
				...
				\(\implies\) Periodische Basislösung \( \begin{bmatrix} \vec{y}_\mathcal{T} \\ \vec{y}_\mathcal{T}^\mathit{co} \end{bmatrix} = \begin{bmatrix} \vec{0} \\ \vec{b} \end{bmatrix} \)
				...
				\begin{minipage}{0.5\textwidth}
					\begin{figure}
						...
						\begin{tikzpicture}[event/.style = { draw, circle }, anchor = center]
							...
							\draw [->] (B) to node[midway, sloped, above]{\( a_1 \)} (A);
							\draw [->] (B) to node[midway, sloped, above]{\( a_2 \)} (C);
							\draw [->] (D) to node[midway, sloped, above]{\( a_3 \)} (A);
							\draw [->] (B) to node[midway, sloped, above]{\( a_4 \)} (D);
							\draw [->] (D) to node[midway, sloped, above]{\( a_5 \)} (C);
							\draw [->] (A) to node[midway, sloped, above]{\( a_6 \)} (C);
						\end{tikzpicture}
				...
				\begin{minipage}{0.5\textwidth}
					\begin{figure}
						...
						\begin{tikzpicture}[event/.style = { draw, circle }, spanning/.style = { draw = tud9b }, co/.style = { draw = tud2b }, anchor = center]
							...
							\draw [->, co] (B) to node[midway, sloped, above]{\( a_1 \)} (A);
							\draw [->, spanning] (B) to node[midway, sloped, above]{\( a_2 \)} (C);
							\draw [->, spanning] (D) to node[midway, sloped, above]{\( a_3 \)} (A);
							\draw [->, co] (B) to node[midway, sloped, above]{\( a_4 \)} (D);
							\draw [->, spanning] (D) to node[midway, sloped, above]{\( a_5 \)} (C);
							\draw [->, co] (A) to node[midway, sloped, above]{\( a_6 \)} (C);
						\end{tikzpicture}
						\caption{{ \color{tud9b} Spannbaum} (rot) und { \color{tud2b} Ko-Baum } (blau)}
					\end{figure}
				\end{minipage}%
				\begin{minipage}{0.5\textwidth}
					\begin{equation*}
				...
				\begin{minipage}{0.5\textwidth}
					\begin{figure}
						...
						\begin{tikzpicture}[event/.style = { draw, circle }, spanning/.style = { draw = tud9b }, co/.style = { draw = tud2b }, anchor = center]
							...
							\draw [->, spanning] (B) to node[midway, sloped, above]{\( a_1 \)} (A);
							\draw [->, co] (B) to node[midway, sloped, above]{\( a_2 \)} (C);
							\draw [->, spanning] (D) to node[midway, sloped, above]{\( a_3 \)} (A);
							\draw [->, co] (B) to node[midway, sloped, above]{\( a_4 \)} (D);
							\draw [->, spanning] (D) to node[midway, sloped, above]{\( a_5 \)} (C);
							\draw [->, co] (A) to node[midway, sloped, above]{\( a_6 \)} (C);
						\end{tikzpicture}
						\caption{Nach Austausch von \(a_1\) und \(a_2\)}
					\end{figure}
				\end{minipage}%
			\end{frame}
		
			\begin{frame}{Beispiel, Iteration 2 Darstellung im Tableau und Kostenänderung}
				...
				\begin{equation*}
					...
					\end{array}
				\end{equation*}
				\(\implies\) keine weitere Verbesserung möglich
			\end{frame}
