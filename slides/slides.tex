	...
	\section{Mathematische Modellierung}
		\subsection{Nicht-periodische Fahrpläne}
			...
			\begin{frame}{Zusammenfassung}
				\begin{itemize}
					\item Fahrplan \( \vec{\pi} = (\pi_i) \) ordnet jedem \(i\) einen Zeitpunkt \( \pi_i \in \R \) zu.
					\item Vorgang \( a : i \to j \) ist der Übergang von \(i\) zu \(j\).
					\item Spannung \( x_a = \pi_j - \pi_i \) ist die Zeit, die ein Vorgang benötigt.
					\item Zulässige Dauer \( \Delta_a = [l_a, u_a] \) mit oberer und unterer Schranke.
				\end{itemize}
			\end{frame}
		% end
